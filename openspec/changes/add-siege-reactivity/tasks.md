...
- [ ] 2.8 Compile-check (IntelliJ `get_file_problems`) and dev-mode verify: weak-victim siege gets victim-led response; alliance victim gets ally-led primary + auxiliaries; failed intervention visibly strains command CR; intervention killing command breaks siege without player bounty
...
- [ ] 5.2 Full-system dev-mode pass: all three systems live on one siege — bounty baits, huntsmen contest interventions when player is unmarked / get pulled off by a marked player, coalition outcomes vary; confirm toggling each `*_ENABLED` off mid-save degrades cleanly
