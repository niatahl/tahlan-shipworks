...
public class DaemonArmor extends BaseHullMod {
...
//        stats.getEffectiveArmorBonus().modifyPercent(id,CALC_PERCENT);
    ...
    public void advanceInCombat(ShipAPI ship, float amount) {
        if (!ship.hasListenerOfClass(DaemonArmorListener.class)) {
            ship.addListener(new DaemonArmorListener());
...
//        if (index == 2) return "" + Math.round(CALC_PERCENT) + txt("%");
    ...
    static class DaemonArmorListener implements DamageTakenModifier {
        @Override
